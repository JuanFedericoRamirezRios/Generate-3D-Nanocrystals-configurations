/*
Generate random NCs with a Gaussian distribution for the size. Some NCs are joined.
*/
...
void GENERATE_NORMAL_DIST_NCS_3D()
{
    new MAIN_FRAME();
}

MAIN_FRAME::MAIN_FRAME()/*  : TGMainFrame(gClient->GetRoot()) */ {
    TString nameOutput = "Co_sputtering_3D_structure.txt";
    Xs = 50;
    Ys = 50;
    Zs = 108;
    meanRadius = 7.69;
    desvRadius = 3.08;
    Int_t seed = 1;
    Int_t thicknessMatNC = 46; //92 pixels is the tickness of silicon.
    ...
    cout << "Desv. standar radius = " << desvRadius << endl;
...
void MAIN_FRAME::PutNC(){
    while(kTRUE) {
        ...
        if(!isRepeat) {
            ...
            nanoCrysts[cryst][3] = randObj->Gaus(meanRadius, desvRadius);
            break;
        } else {

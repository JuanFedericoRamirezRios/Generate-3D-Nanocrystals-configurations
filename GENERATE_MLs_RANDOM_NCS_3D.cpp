/*
Generate random NCs structure in MLs, every NCs are the same size. The NCs are separated.
*/
...
public:
    MAIN_FRAME();
    Bool_t PutNC(Int_t yPosition, Double_t radii);
...
void GENERATE_MLs_RANDOM_NCS_3D()
{
    new MAIN_FRAME();
}

MAIN_FRAME::MAIN_FRAME()/*  : TGMainFrame(gClient->GetRoot()) */ {
    TString nameOutput = "9s1_3D_structure.txt";
    Xs = 50;
    Ys = 50;
    Zs = 231;
    meanRadius = 8;
    Int_t seed = 1;
    Int_t zCenterRadiusMLs[9][2] = {    // zCenterRadiusMLs[][0]: z coordenate of centers 
                                        // zCenterRadiusMLs[][1]: radius
        {17, (Int_t)meanRadius},
        {42, (Int_t)meanRadius},
        {66, (Int_t)meanRadius},
        {91, (Int_t)meanRadius},
        {115, (Int_t)meanRadius},
        {140, (Int_t)meanRadius},
        {165, (Int_t)meanRadius},
        {189, (Int_t)meanRadius},
        {214, (Int_t)meanRadius},
    };
    maxAttempts = Xs*Ys;
    ...
    for(Int_t zCenter = 0; zCenter < (sizeof(zCenterRadiusMLs)/sizeof(zCenterRadiusMLs[0])); zCenter++) {
        for(Int_t n = 0; n < numNCsByLayer; n++) {
            ...
            PutNC(zCenterRadiusMLs[zCenter][0], zCenterRadiusMLs[zCenter][1]);
        }
    }
...
Bool_t MAIN_FRAME::PutNC(Int_t zPosition, Double_t radii){
    Int_t attempt = 0;

/*
Generate random NCs structure in MLs, the size have a Gaussian (normal) distribution. The NCs are separated.
*/
#include "TF1.h"
#include "TH2F.h"
...
void GENERATE_MLs_NORMAL_DIST_NCS_3D()
{
    new MAIN_FRAME();
}

MAIN_FRAME::MAIN_FRAME()/*  : TGMainFrame(gClient->GetRoot()) */ {    
    TString nameOutput = "1BL_3D_Miguel_NormalDist-structure.txt";
    Xs = 50;
    Ys = 50;
    Zs = 38;
    meanRadius = 6.7;
    desvRadius = 0.8;
    Int_t seed = 1;
    Double_t zCenterRadiusMLs[1][3] = {    // zCenterRadiusMLs[][0]: z coordenate of centers 
                                        // zCenterRadiusMLs[][1]: mean radius
                                        // zCenterRadiusMLs[][2]: Desv. standar of radius
        {26, meanRadius, desvRadius}
    };
    maxAttempts = Xs*Ys;
